...
<div class="block">
  {#each data.activiteiten.items as act}
    <div
      class="grid grid-cols-1 md:grid-cols-3 w-9/12 my-4 mx-auto px-4 py-3 align-middle gap-8 rounded-xl activiteit"
    >
      <div>
        <a href={"/activiteiten/" + act.slug} class="hover:text-red-600"
          >{act.titel}</a
        >
      </div>
...
<!--Started using style tags for better oversight-->
<style>
  .activiteit {
